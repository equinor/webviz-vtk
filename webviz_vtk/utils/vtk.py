    ...
    if (
        ...
        if test_value < np.iinfo(np.int16).max and signed:
            ...
            buffer = base64.b64encode(
                memoryview(obj.astype(np.int16).ravel(order="C"))
            ).decode("utf-8")
        elif test_value < np.iinfo(np.int32).max and signed:
            dtype_str = "int32"
            buffer = base64.b64encode(
                memoryview(obj.astype(np.int32).ravel(order="C"))
            ).decode("utf-8")
        elif test_value < np.iinfo(np.uint16).max and not signed:
            dtype_str = "uint16"
            buffer = base64.b64encode(
                memoryview(obj.astype(np.uint16).ravel(order="C"))
            ).decode("utf-8")
        elif test_value < np.iinfo(np.uint32).max and not signed:
            dtype_str = "uint32"
            buffer = base64.b64encode(
                memoryview(obj.astype(np.uint32).ravel(order="C"))
            ).decode("utf-8")
